...
class DatabaseQueryBuilderTest extends TestCase
{
    ...
    public function testUpsertMethod()
    {
        ...
        $builder->getConnection()
            ...
            ->shouldReceive('affectingStatement')->once()->with('insert into `users` (`email`, `name`) values (?, ?), (?, ?) as kenoura_upsert_alias on duplicate key update `email` = `kenoura_upsert_alias`.`email`, `name` = `kenoura_upsert_alias`.`name`', ['foo', 'bar', 'foo2', 'bar2'])->andReturn(2);
        $result = $builder->from('users')->upsert([['email' => 'foo', 'name' => 'bar'], ['name' => 'bar2', 'email' => 'foo2']], 'email');
        $this->assertEquals(2, $result);
        ...
        $builder = $this->getSqlServerBuilder();
        $builder->getConnection()->shouldReceive('affectingStatement')->once()->with('merge [users] using (values (?, ?), (?, ?)) [kenoura_source] ([email], [name]) on [kenoura_source].[email] = [users].[email] when matched then update set [email] = [kenoura_source].[email], [name] = [kenoura_source].[name] when not matched then insert ([email], [name]) values ([email], [name]);', ['foo', 'bar', 'foo2', 'bar2'])->andReturn(2);
        $result = $builder->from('users')->upsert([['email' => 'foo', 'name' => 'bar'], ['name' => 'bar2', 'email' => 'foo2']], 'email');
        $this->assertEquals(2, $result);
    }
    ...
    public function testUpsertMethodWithUpdateColumns()
    {
        ...
        $builder->getConnection()
            ->shouldReceive('getConfig')->with('use_upsert_alias')->andReturn(true)
            ->shouldReceive('affectingStatement')->once()->with('insert into `users` (`email`, `name`) values (?, ?), (?, ?) as kenoura_upsert_alias on duplicate key update `name` = `kenoura_upsert_alias`.`name`', ['foo', 'bar', 'foo2', 'bar2'])->andReturn(2);
        $result = $builder->from('users')->upsert([['email' => 'foo', 'name' => 'bar'], ['name' => 'bar2', 'email' => 'foo2']], 'email', ['name']);
        $this->assertEquals(2, $result);
        ...
        $builder = $this->getSqlServerBuilder();
        $builder->getConnection()->shouldReceive('affectingStatement')->once()->with('merge [users] using (values (?, ?), (?, ?)) [kenoura_source] ([email], [name]) on [kenoura_source].[email] = [users].[email] when matched then update set [name] = [kenoura_source].[name] when not matched then insert ([email], [name]) values ([email], [name]);', ['foo', 'bar', 'foo2', 'bar2'])->andReturn(2);
        $result = $builder->from('users')->upsert([['email' => 'foo', 'name' => 'bar'], ['name' => 'bar2', 'email' => 'foo2']], 'email', ['name']);
        $this->assertEquals(2, $result);
    }
    ...
    public function testPreservedAreAppliedByUpsert()
    {
        ...
        $builder->getConnection()
            ->shouldReceive('getConfig')->with('use_upsert_alias')->andReturn(true)
            ->shouldReceive('affectingStatement')->once()->with('insert into `users` (`email`) values (?) as kenoura_upsert_alias on duplicate key update `email` = `kenoura_upsert_alias`.`email`', ['foo']);
        $builder->beforeQuery(function ($builder) {
            $builder->from('users');
        });
        $builder->upsert(['email' => 'foo'], 'id');
    ...
    public function testMySqlUpdateWrappingJsonArray()
    {
        ...
        $connection->expects($this->once())
            ...
            ->with(
                ...
                [
                    json_encode(['2fa' => false, 'presets' => ['kenoura', 'vue']]),
        ...
        $builder->from('users')->where('active', 1)->update([
            'options' => ['2fa' => false, 'presets' => ['kenoura', 'vue']],
    ...
    public function testSQLiteUpdateWrappingNestedJsonArray()
    {
        ...
        $builder->getConnection()->shouldReceive('update')
            ->with('update "users" set "group_id" = 45, "created_at" = ?, "options" = json_patch(ifnull("options", json(\'{}\')), json(?))', [
                new DateTime('2019-08-06'),
                json_encode(['name' => 'Taylor', 'security' => ['2fa' => false, 'presets' => ['kenoura', 'vue']], 'sharing' => ['twitter' => 'username']]),
            ]);

        $builder->from('users')->update([
            'options->name' => 'Taylor',
            'group_id' => new Raw('45'),
            'options->security' => ['2fa' => false, 'presets' => ['kenoura', 'vue']],
