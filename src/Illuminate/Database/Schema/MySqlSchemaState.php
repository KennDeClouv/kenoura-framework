...
class MySqlSchemaState extends SchemaState
{
    ...
    public function dump(Connection $connection, $path)
    {
        $this->executeDumpProcess($this->makeProcess(
            $this->baseDumpCommand().' --routines --result-file="${:KENOURA_LOAD_PATH}" --no-data'
    ...
    {
        $command = 'mysql '.$this->connectionString().' --database="${:KENOURA_LOAD_DATABASE}" < "${:KENOURA_LOAD_PATH}"';
    ...
    protected function baseDumpCommand()
    {
        ...
        return $command.' "${:KENOURA_LOAD_DATABASE}"';
    }
    ...
    {
        $value = ' --user="${:KENOURA_LOAD_USER}" --password="${:KENOURA_LOAD_PASSWORD}"';

        $config = $this->connection->getConfig();

        $value .= $config['unix_socket'] ?? false
                        ? ' --socket="${:KENOURA_LOAD_SOCKET}"'
                        : ' --host="${:KENOURA_LOAD_HOST}" --port="${:KENOURA_LOAD_PORT}"';

        if (isset($config['options'][\PDO::MYSQL_ATTR_SSL_CA])) {
            $value .= ' --ssl-ca="${:KENOURA_LOAD_SSL_CA}"';
        }
    ...
    protected function baseVariables(array $config)
    {
        ...
        return [
            'KENOURA_LOAD_SOCKET' => $config['unix_socket'] ?? '',
            ...
            'KENOURA_LOAD_PASSWORD' => $config['password'] ?? '',
            'KENOURA_LOAD_DATABASE' => $config['database'],
            'KENOURA_LOAD_SSL_CA' => $config['options'][\PDO::MYSQL_ATTR_SSL_CA] ?? '',
        ];
    }
