...
use function Kenoura\Prompts\confirm;
...
class BroadcastingInstallCommand extends Command
{
    ...
    protected $signature = 'install:broadcasting
                    ...
                    {--without-reverb : Do not prompt to install Kenoura Reverb}
    ...
    public function handle()
    {
        ...
        if (! file_exists($broadcastingRoutesPath = $this->kenoura->basePath('routes/channels.php')) || $this->option('force')) {
        ...
        if (! file_exists($echoScriptPath = $this->kenoura->resourcePath('js/echo.js'))) {
            if (! is_dir($directory = $this->kenoura->resourcePath('js'))) {
        ...
        if (file_exists($bootstrapScriptPath = $this->kenoura->resourcePath('js/bootstrap.js'))) {
    ...
    /**
     * Install Kenoura Reverb into the application if desired.
     *
     * @return void
     */
    protected function installReverb()
    {
        if ($this->option('without-reverb') || InstalledVersions::isInstalled('kenoura/reverb')) {
            return;
        }

        $install = confirm('Would you like to install Kenoura Reverb?', default: true);
    ...
    protected function installNodeDependencies()
    {
        ...
        if (file_exists(base_path('pnpm-lock.yaml'))) {
            $commands = [
                'pnpm add --save-dev kenoura-echo pusher-js',
                'pnpm run build',
            ];
        } elseif (file_exists(base_path('yarn.lock'))) {
            $commands = [
                'yarn add --dev kenoura-echo pusher-js',
                'yarn run build',
            ];
        } elseif (file_exists(base_path('bun.lock')) || file_exists(base_path('bun.lockb'))) {
            $commands = [
                'bun add --dev kenoura-echo pusher-js',
                'bun run build',
            ];
        } else {
            $commands = [
