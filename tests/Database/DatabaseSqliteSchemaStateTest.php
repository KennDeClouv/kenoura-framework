...
class DatabaseSqliteSchemaStateTest extends TestCase
{
    ...
    public function testLoadSchemaToDatabase(): void
    {
        ...
        $processFactory->shouldHaveBeenCalled()->with('sqlite3 "${:KENOURA_LOAD_DATABASE}" < "${:KENOURA_LOAD_PATH}"');

        $process->shouldHaveReceived('mustRun')->with(null, [
            'KENOURA_LOAD_DATABASE' => 'database/database.sqlite',
            'KENOURA_LOAD_PATH' => 'database/schema/sqlite-schema.dump',
        ]);
    }
