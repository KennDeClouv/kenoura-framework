...
class WorkCommand extends Command
{
    ...
    public function handle()
    {
        ...
        $connection = $this->argument('connection')
                        ?: $this->kenoura['config']['queue.default'];
    ...
    protected function listenForEvents()
    {
        ...
        $this->kenoura['events']->listen(JobProcessing::class, function ($event) {
            $this->writeOutput($event->job, 'starting');
        });

        $this->kenoura['events']->listen(JobProcessed::class, function ($event) {
            $this->writeOutput($event->job, 'success');
        });

        $this->kenoura['events']->listen(JobReleasedAfterException::class, function ($event) {
            $this->writeOutput($event->job, 'released_after_exception');
        });

        $this->kenoura['events']->listen(JobFailed::class, function ($event) {
    ...
    {
        $queueTimezone = $this->kenoura['config']->get('queue.output_timezone');

        if ($queueTimezone &&
            $queueTimezone !== $this->kenoura['config']->get('app.timezone')) {
    ...
    {
        return $this->option('queue') ?: $this->kenoura['config']->get(
    ...
    {
        return $this->option('force') ? false : $this->kenoura->isDownForMaintenance();
    }
