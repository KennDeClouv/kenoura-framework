...
trait InteractsWithUniqueJobs
{
    ...
    public function addUniqueJobInformationToContext($job): void
    {
        if ($job instanceof ShouldBeUnique) {
            Context::addHidden([
                'kenoura_unique_job_cache_store' => $this->getUniqueJobCacheStore($job),
                'kenoura_unique_job_key' => UniqueLock::getKey($job),
            ]);
        }
    }
    ...
    public function removeUniqueJobInformationFromContext($job): void
    {
        if ($job instanceof ShouldBeUnique) {
            Context::forgetHidden([
                'kenoura_unique_job_cache_store',
                'kenoura_unique_job_key',
            ]);
        }
    }
