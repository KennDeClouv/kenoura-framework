...
class StorageLinkCommand extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($this->links() as $link => $target) {
            ...
            if (is_link($link)) {
                $this->kenoura->make('files')->delete($link);
            }

            if ($relative) {
                $this->kenoura->make('files')->relativeLink($target, $link);
            } else {
                $this->kenoura->make('files')->link($target, $link);
            }
    ...
    {
        return $this->kenoura['config']['filesystems.links'] ??
