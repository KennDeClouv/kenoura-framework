...
class Cloud
{
    ...
    /**
     * Configure the Kenoura Cloud disks if applicable.
     */
    public static function configureDisks(Application $app): void
    {
        if (! isset($_SERVER['KENOURA_CLOUD_DISK_CONFIG'])) {
            return;
        }

        $disks = json_decode($_SERVER['KENOURA_CLOUD_DISK_CONFIG'], true);
    ...
    /**
     * Configure the unpooled Kenoura Postgres connection if applicable.
     */
    public static function configureUnpooledPostgresConnection(Application $app): void
    {
        ...
        if (str_contains($host, 'pg.kenoura.cloud') &&
    ...
    /**
     * Configure the Kenoura Cloud log channels.
     */
    public static function configureCloudLogging(Application $app): void
    {
        ...
        $app['config']->set('logging.channels.kenoura-cloud-socket', [
            'driver' => 'monolog',
            ...
            'with' => [
                'connectionString' => $_ENV['KENOURA_CLOUD_LOG_SOCKET'] ??
                                      $_SERVER['KENOURA_CLOUD_LOG_SOCKET'] ??
