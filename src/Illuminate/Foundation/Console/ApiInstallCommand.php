...
class ApiInstallCommand extends Command
{
    ...
    protected $signature = 'install:api
                    ...
                    {--passport : Install Kenoura Passport instead of Kenoura Sanctum}
     ...
     * @var string
     */
    protected $description = 'Create an API routes file and install Kenoura Sanctum or Kenoura Passport';
    ...
    public function handle()
    {
        ...
        if (file_exists($apiRoutesPath = $this->kenoura->basePath('routes/api.php')) &&
            ! $this->option('force')) {
        ...
        if ($this->option('passport')) {
            ...
            $this->components->info('API scaffolding installed. Please add the [Kenoura\Passport\HasApiTokens] trait to your User model.');
        } else {
            ...
            $this->components->info('API scaffolding installed. Please add the [Kenoura\Sanctum\HasApiTokens] trait to your User model.');
        }
    }
    ...
    {
        $appBootstrapPath = $this->kenoura->bootstrapPath('app.php');
    ...
    /**
     * Install Kenoura Sanctum into the application.
     *
     * @return void
     */
    protected function installSanctum()
    {
        ...
        $migrationPublished = (new Collection(scandir($this->kenoura->databasePath('migrations'))))->contains(function ($migration) {
        ...
        if (! $migrationPublished) {
            Process::run([
                ...
                'vendor:publish',
                '--provider',
                'Kenoura\\Sanctum\\SanctumServiceProvider',
            ]);
        }
    }

    /**
     * Install Kenoura Passport into the application.
     *
     * @return void
     */
