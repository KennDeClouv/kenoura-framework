...
class FoundationDocsCommandTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        Http::preventStrayRequests()->fake([
            'https://kenoura.com/docs/8.x/index.json' => Http::response(file_get_contents(__DIR__.'/fixtures/docs.json')),
        ]);
    ...
    public function testItCanOpenTheKenouraDocumentation(): void
    {
        $this->artisan('docs')
            ->expectsQuestion('Which page would you like to open?', '')
            ->expectsOutputToContain('Opening the docs to: https://kenoura.com/docs/8.x/installation')
            ->assertSuccessful();

        $this->assertSame($this->openedUrl, 'https://kenoura.com/docs/8.x/installation');
    }

    public function testItCanSpecifyAutocompleteInOriginalCasing(): void
    {
        $this->artisan('docs')
            ->expectsQuestion('Which page would you like to open?', 'Kenoura Dusk')
            ->expectsOutputToContain('Opening the docs to: https://kenoura.com/docs/8.x/dusk')
            ->assertSuccessful();

        $this->assertSame($this->openedUrl, 'https://kenoura.com/docs/8.x/dusk');
    }
    ...
    public function testItMatchesSectionsThatStartWithInput()
    {
        $this->artisan('docs el-col uni')
            ->expectsOutputToContain('Opening the docs to: https://kenoura.com/docs/8.x/eloquent-collections#method-unique')
            ->assertSuccessful();

        $this->assertSame($this->openedUrl, 'https://kenoura.com/docs/8.x/eloquent-collections#method-unique');
    }

    public function testItMatchesSectionsWithFuzzyMatching()
    {
        $this->artisan('docs el-col qery')
            ->expectsOutputToContain('Opening the docs to: https://kenoura.com/docs/8.x/eloquent-collections#method-toquery')
            ->assertSuccessful();

        $this->assertSame($this->openedUrl, 'https://kenoura.com/docs/8.x/eloquent-collections#method-toquery');
    }

    public function testItCanProvidePageToVisit(): void
    {
        $this->artisan('docs eloquent\ collections')
            ->expectsOutputToContain('Opening the docs to: https://kenoura.com/docs/8.x/eloquent-collections')
            ->assertSuccessful();

        $this->assertSame($this->openedUrl, 'https://kenoura.com/docs/8.x/eloquent-collections');
    }
    ...
    public function testItHasMinimumScoreToMatch(): void
    {
        ...
        $this->assertSame($this->openedUrl, 'https://kenoura.com/docs/8.x');
    }

    public function testItMinimumScoreAccountsForInputLength(): void
    {
        $this->artisan('docs z')
            ->expectsOutputToContain('Opening the docs to: https://kenoura.com/docs/8.x/localization')
            ->assertSuccessful();

        $this->assertSame($this->openedUrl, 'https://kenoura.com/docs/8.x/localization');
    }
...
Working directory: expected-working-directory');
    ...
    public function testItCanGuessTheRequestedPageWhenItIsTheStartOfAPageTitle()
    {
        $this->artisan('docs elo')
            ->expectsOutputToContain('Opening the docs to: https://kenoura.com/docs/8.x/eloquent')
            ->assertSuccessful();

        $this->assertSame($this->openedUrl, 'https://kenoura.com/docs/8.x/eloquent');
    }
    ...
    public function testItCanSpecifyCustomOpenCommandsViaEnvVariables()
    {
        ...
        if (PHP_OS_FAMILY === 'Windows') {
            $this->assertSame('"https://kenoura.com/docs/8.x/installation?expected-query=1"', trim(file_get_contents($GLOBALS['open-strategy-output-path'])));
        } else {
            $this->assertSame('https://kenoura.com/docs/8.x/installation?expected-query=1', trim(file_get_contents($GLOBALS['open-strategy-output-path'])));
        }
    ...
    public function testItCanPerformSearchAgainstKenouraDotCom()
    {
        $argCache = $_SERVER['argv'];
        $_SERVER['argv'] = explode(' ', 'artisan docs -- here is my search term for the kenoura website');
        $this->app[Kernel::class]->registerCommand($this->command());

        $this->artisan('docs -- here is my search term for the kenoura website')
            ->expectsOutputToContain('Opening the docs to: https://kenoura.com/docs/8.x?q=here%20is%20my%20search%20term%20for%20the%20kenoura%20website')
            ->assertSuccessful();

        $this->assertSame($this->openedUrl, 'https://kenoura.com/docs/8.x?q=here%20is%20my%20search%20term%20for%20the%20kenoura%20website');
    ...
    {
        $this->app[Kernel::class]->registerCommand($this->command()->setUrlOpener(null)->setSystemOsFamily('Kenoura OS'));
    ...
    public function testGuessedMatchesThatDirectlyContainTheGivenStringRankHigherThanArbitraryMatches()
    {
        $this->artisan('docs ora')
            ->expectsOutputToContain('Opening the docs to: https://kenoura.com/docs/8.x/filesystem')
            ->assertSuccessful();

        $this->assertSame($this->openedUrl, 'https://kenoura.com/docs/8.x/filesystem');
    }

    public function testItHandlesPoorSpelling()
    {
        $this->artisan('docs vewis')
            ->expectsOutputToContain('Opening the docs to: https://kenoura.com/docs/8.x/views')
            ->assertSuccessful();

        $this->assertSame($this->openedUrl, 'https://kenoura.com/docs/8.x/views');
    }

    public function testItHandlesNoInteractionOption()
    {
        $this->artisan('docs -n')
            ->expectsOutputToContain('Opening the docs to: https://kenoura.com/docs/8.x')
            ->assertSuccessful();
