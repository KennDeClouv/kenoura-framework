...
class FacadesTest extends TestCase
{
    protected function tearDown(): void
    {
        ...
        unset($_SERVER['__kenoura.authResolved']);
    }

    public function testFacadeResolvedCanResolveCallback()
    {
        Auth::resolved(function (AuthManager $auth, Application $app) {
            $_SERVER['__kenoura.authResolved'] = true;
        });

        $this->assertFalse(isset($_SERVER['__kenoura.authResolved']));

        $this->app->make('auth');

        $this->assertTrue(isset($_SERVER['__kenoura.authResolved']));
    }
