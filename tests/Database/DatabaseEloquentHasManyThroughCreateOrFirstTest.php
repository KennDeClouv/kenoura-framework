...
class DatabaseEloquentHasManyThroughCreateOrFirstTest extends TestCase
{
    ...
    public function testCreateOrFirstMethodRetrievesExistingRecord(): void
    {
        ...
        $parent->getConnection()
            ->expects('select')
            ->with(
                'select "child".*, "pivot"."parent_id" as "kenoura_through_key" from "child" inner join "pivot" on "pivot"."id" = "child"."pivot_id" where "pivot"."parent_id" = ? and ("attr" = ?) limit 1',
                [123, 'foo'],
                true,
            )
            ->andReturn([[
    ...
    public function testFirstOrCreateMethodRetrievesRecordCreatedJustNow(): void
    {
        ...
        $parent->getConnection()
            ->expects('select')
            ->with(
                'select "child".*, "pivot"."parent_id" as "kenoura_through_key" from "child" inner join "pivot" on "pivot"."id" = "child"."pivot_id" where "pivot"."parent_id" = ? and ("attr" = ? and "val" = ?) limit 1',
                [123, 'foo', 'bar'],
                true,
            )
            ->andReturn([[
