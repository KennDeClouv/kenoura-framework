...
class ConfigPublishCommand extends Command
{
    ...
    public function handle()
    {
        ...
        if (is_null($this->argument('name')) && $this->option('all')) {
            foreach ($config as $key => $file) {
                $this->publish($key, $file, $this->kenoura->configPath().'/'.$key.'.php');
            }
        ...
        $this->publish($name, $config[$name], $this->kenoura->configPath().'/'.$name.'.php');
    }
    ...
    protected function getBaseConfigurationFiles()
    {
        ...
        $shouldMergeConfiguration = $this->kenoura->shouldMergeFrameworkConfiguration();
