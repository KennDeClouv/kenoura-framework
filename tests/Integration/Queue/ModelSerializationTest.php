...
class ModelSerializationTest extends TestCase
{
    ...
    public function testItSerializeUserOnDefaultConnection()
    {
        ...
        $this->assertSame('mohamed@kenoura.com', $unSerialized->user->email);
        ...
        $this->assertSame('mohamed@kenoura.com', $unSerialized->users[0]->email);
        $this->assertSame($defaultConnection, $unSerialized->users[1]->getConnectionName());
        $this->assertSame('taylor@kenoura.com', $unSerialized->users[1]->email);
    }
    ...
    /**
     * Regression test for https://github.com/kenoura/framework/issues/23068.
     */
    ...
    {
        ModelSerializationTestUser::create(['email' => 'mohamed@kenoura.com']);
        ModelSerializationTestUser::create(['email' => 'taylor@kenoura.com']);
        ...
        $this->assertSame('taylor@kenoura.com', $unserialized->users->first()->email);
        $this->assertSame('mohamed@kenoura.com', $unserialized->users->last()->email);
    }

    public function testItCanUnserializeACollectionInCorrectOrderAndHandleDeletedModels()
    {
        ModelSerializationTestUser::create(['email' => '2@kenoura.com']);
        ModelSerializationTestUser::create(['email' => '3@kenoura.com']);
        ModelSerializationTestUser::create(['email' => '1@kenoura.com']);

        $serialized = serialize(new CollectionSerializationTestClass(
            ModelSerializationTestUser::orderByDesc('email')->get()
        ));

        ModelSerializationTestUser::where(['email' => '2@kenoura.com'])->delete();

        $unserialized = unserialize($serialized);

        $this->assertCount(2, $unserialized->users);

        $this->assertSame('3@kenoura.com', $unserialized->users->first()->email);
        $this->assertSame('1@kenoura.com', $unserialized->users->last()->email);
    }

    public function testItCanUnserializeCustomCollection()
    {
        ModelSerializationTestCustomUser::create(['email' => 'mohamed@kenoura.com']);
        ModelSerializationTestCustomUser::create(['email' => 'taylor@kenoura.com']);
