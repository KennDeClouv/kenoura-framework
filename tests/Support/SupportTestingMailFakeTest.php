...
class SupportTestingMailFakeTest extends TestCase
{
    ...
    public function testAssertSent()
    {
        ...
        $this->fake->to('taylor@kenoura.com')->send($this->mailable);
    ...
    {
        try {
            $this->fake->assertSent(MailableStub::class, 'taylor@kenoura.com');
            $this->fail();
        } catch (ExpectationFailedException $e) {
            $this->assertStringContainsString('The expected [Illuminate\Tests\Support\MailableStub] mailable was not sent to address [taylor@kenoura.com].', $e->getMessage());
        }
    ...
    {
        $this->fake->to('dries@kenoura.com')->send($this->mailable);
        $this->fake->to('taylor@kenoura.com')->send($this->mailable);

        $this->fake->to(['nuno@kenoura.com', 'jess@kenoura.com'])->send($this->mailable);

        $this->fake->assertSent(MailableStub::class, 3);
        $this->fake->assertSent(
            MailableStub::class,
            ['taylor@kenoura.com', 'dries@kenoura.com', 'nuno@kenoura.com', 'jess@kenoura.com']
        );
    }
    ...
    public function testAssertTo()
    {
        ...
        $this->fake->assertSent(MailableStub::class, function ($mail) {
            return $mail->hasTo('taylor@kenoura.com');
        });
    }

    public function testAssertCc()
    {
        $this->fake->cc('taylor@kenoura.com')->send($this->mailable);

        $this->fake->assertSent(MailableStub::class, function ($mail) {
            return $mail->hasCc('taylor@kenoura.com');
        });
    }

    public function testAssertBcc()
    {
        $this->fake->bcc('taylor@kenoura.com')->send($this->mailable);

        $this->fake->assertSent(MailableStub::class, function ($mail) {
            return $mail->hasBcc('taylor@kenoura.com');
        });
    }
    ...
    {
        $this->fake->assertNotSent(MailableStub::class, 'taylor@kenoura.com');

        $this->fake->to('taylor@kenoura.com')->send($this->mailable);

        $this->expectException(ExpectationFailedException::class);
        $this->expectExceptionMessage('The unexpected ['.MailableStub::class.'] mailable was sent to address [taylor@kenoura.com].');
    ...
    {
        $this->fake->assertNotSent(MailableStub::class, ['taylor@kenoura.com', 'dries@kenoura.com']);

        $this->fake->to('dries@kenoura.com')->send($this->mailable);

        $this->expectException(ExpectationFailedException::class);
        $this->expectExceptionMessage('The unexpected ['.MailableStub::class.'] mailable was sent to address [dries@kenoura.com].');
    ...
    public function testAssertQueued()
    {
        ...
        $this->fake->to('taylor@kenoura.com')->queue($this->mailable);
    ...
    {
        try {
            $this->fake->assertQueued(MailableStub::class, 'taylor@kenoura.com');
            $this->fail();
        } catch (ExpectationFailedException $e) {
            $this->assertStringContainsString('The expected [Illuminate\Tests\Support\MailableStub] mailable was not queued to address [taylor@kenoura.com].', $e->getMessage());
        }
    ...
    {
        $this->fake->to('dries@kenoura.com')->queue($this->mailable);
        $this->fake->to('taylor@kenoura.com')->queue($this->mailable);

        $this->fake->to(['nuno@kenoura.com', 'jess@kenoura.com'])->queue($this->mailable);
    ...
    {
        $this->fake->assertNotQueued(MailableStub::class, 'taylor@kenoura.com');

        $this->fake->to('taylor@kenoura.com')->queue($this->mailable);

        $this->expectException(ExpectationFailedException::class);
        $this->expectExceptionMessage('The unexpected ['.MailableStub::class.'] mailable was queued to address [taylor@kenoura.com].');
    ...
    {
        $this->fake->assertNotQueued(MailableStub::class, ['taylor@kenoura.com', 'dries@kenoura.com']);

        $this->fake->to('dries@kenoura.com')->queue($this->mailable);

        $this->expectException(ExpectationFailedException::class);
        $this->expectExceptionMessage('The unexpected ['.MailableStub::class.'] mailable was queued to address [dries@kenoura.com].');
    ...
    {
        $this->fake->to('taylor@kenoura.com')->send(new QueueableMailableStub);
