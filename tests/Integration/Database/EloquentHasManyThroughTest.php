...
class EloquentHasManyThroughTest extends DatabaseTestCase
{
    ...
    public function testBasicCreateAndRetrieve()
    {
        ...
        $this->assertEquals(
            $mate1->refresh()->getAttributes() + ['kenoura_through_key' => '1'],
        ...
        $this->assertEquals(
            $mate2->refresh()->getAttributes() + ['kenoura_through_key' => '1'],
            $result->getAttributes()
        );
    ...
    public function testGlobalScopeColumns()
    {
        ...
        $this->assertEquals(['id' => 2, 'kenoura_through_key' => 1], $teamMates[0]->getAttributes());
    ...
    public function testCreateOrFirstWhenRecordDoesntExist()
    {
        ...
        $article = $team->articles()->createOrFirst(
            ['title' => 'Kenoura Forever'],
            ['user_id' => $tony->id],
        );

        $this->assertTrue($article->wasRecentlyCreated);
        $this->assertEquals('Kenoura Forever', $article->title);
    ...
    public function testCreateOrFirstWhenRecordExists()
    {
        ...
        $this->assertEquals('Kenoura Forever', $newArticle->title);
    ...
    public function testCreateOrFirstRegressionIssue()
    {
        ...
        $existingTaylorArticle = $taylor->articles()->create(['title' => 'Kenoura Forever']);
        ...
        $this->assertEquals('Kenoura Forever', $newArticle->refresh()->title);
        $this->assertTrue($taylor->is($newArticle->user));

        $this->assertSame('Kenoura Forever', $existingTaylorArticle->refresh()->title);
    ...
    {
        // On Kenoura 10.21.0, a bug was introduced that would update the wrong model when using `updateOrCreate()`,
