...
class TestWithoutDatabaseParallelTest extends TestCase
{
    ...
    protected function defineEnvironment($app)
    {
        ...
        $_SERVER['KENOURA_PARALLEL_TESTING'] = 1;
        $_SERVER['KENOURA_PARALLEL_TESTING_WITHOUT_DATABASES'] = 1;
        $_SERVER['TEST_TOKEN'] = '1';

        $this->beforeApplicationDestroyed(function () {
            unset(
                $_SERVER['KENOURA_PARALLEL_TESTING'],
                $_SERVER['KENOURA_PARALLEL_TESTING_WITHOUT_DATABASES'],
                $_SERVER['TEST_TOKEN'],
            );
        });
    }
