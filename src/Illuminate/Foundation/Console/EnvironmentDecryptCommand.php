...
use function Kenoura\Prompts\password;
...
class EnvironmentDecryptCommand extends Command
{
    ...
    {
        $key = $this->option('key') ?: Env::get('KENOURA_ENV_ENCRYPTION_KEY');
        ...
        $encryptedFile = ($this->option('env')
                    ? Str::finish(dirname($this->kenoura->environmentFilePath()), DIRECTORY_SEPARATOR).'.env.'.$this->option('env')
                    : $this->kenoura->environmentFilePath()).'.encrypted';
    ...
    {
        $path = Str::finish($this->option('path') ?: dirname($this->kenoura->environmentFilePath()), DIRECTORY_SEPARATOR);
