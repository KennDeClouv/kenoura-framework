...
class SqlServerGrammar extends Grammar
{
    ...
    public function compileUpsert(Builder $query, array $values, array $uniqueBy, array $update)
    {
        ...
        $sql .= 'using (values '.$parameters.') '.$this->wrapTable('kenoura_source').' ('.$columns.') ';

        $on = (new Collection($uniqueBy))->map(function ($column) use ($query) {
            return $this->wrap('kenoura_source.'.$column).' = '.$this->wrap($query->from.'.'.$column);
        })->implode(' and ');
        ...
        if ($update) {
            $update = (new Collection($update))->map(function ($value, $key) {
                return is_numeric($key)
                    ? $this->wrap($value).' = '.$this->wrap('kenoura_source.'.$value)
