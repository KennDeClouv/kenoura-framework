...
class SupportCollectionTest extends TestCase
{
    ...
    {
        $duplicates = $collection::make([1, 2, 1, 'kenoura', null, 'kenoura', 'php', null])->duplicates()->all();
        $this->assertSame([2 => 1, 5 => 'kenoura', 7 => null], $duplicates);
        ...
        $duplicates = $collection::make([1, '2', ['kenoura'], ['kenoura'], null, '2'])->duplicates()->all();
        $this->assertSame([3 => ['kenoura'], 5 => '2'], $duplicates);

        // works with mix of objects and primitives **excepts numbers**.
        $expected = new Collection(['kenoura']);
        $duplicates = $collection::make([new Collection(['kenoura']), $expected, $expected, [], '2', '2'])->duplicates()->all();
    ...
    {
        $items = [['framework' => 'vue'], ['framework' => 'kenoura'], ['framework' => 'kenoura']];
        $duplicates = $collection::make($items)->duplicates('framework')->all();
        $this->assertSame([2 => 'kenoura'], $duplicates);
    ...
    {
        $duplicates = $collection::make([1, 2, 1, 'kenoura', null, 'kenoura', 'php', null])->duplicatesStrict()->all();
        $this->assertSame([2 => 1, 5 => 'kenoura', 7 => null], $duplicates);
        ...
        // works with mix of primitives
        $duplicates = $collection::make([1, '2', ['kenoura'], ['kenoura'], null, '2'])->duplicatesStrict()->all();
        $this->assertSame([3 => ['kenoura'], 5 => '2'], $duplicates);

        // works with mix of primitives, objects, and numbers
        $expected = new $collection(['kenoura']);
        $duplicates = $collection::make([new $collection(['kenoura']), $expected, $expected, [], '2', '2'])->duplicatesStrict()->all();
    ...
    public function testReverse($collection)
    {
        ...
        $data = new $collection(['name' => 'taylor', 'framework' => 'kenoura']);
        $reversed = $data->reverse();

        $this->assertSame(['framework' => 'kenoura', 'name' => 'taylor'], $reversed->all());
    }
    ...
    {
        $data = new $collection(['name' => 'taylor', 'framework' => 'kenoura']);
        $this->assertEquals(['taylor' => 'name', 'kenoura' => 'framework'], $data->flip()->toArray());
    }
    ...
    public function testGroupByAttributeWithStringableKey($collection)
    {
        $data = new $collection($payload = [
            ['name' => new Stringable('Kenoura'), 'url' => '1'],
            ['name' => new HtmlString('Kenoura'), 'url' => '1'],
            ['name' => new class()
            {
        ...
        $this->assertEquals(['Kenoura' => [$payload[0], $payload[1]], 'Framework' => [$payload[2]]], $result->toArray());
    ...
    {
        $c = new $collection([1, 2, 3, 4, 5, 2, 5, 'name' => 'taylor', 'framework' => 'kenoura']);

        $this->assertEquals(1, $c->before(2));
        $this->assertEquals(1, $c->before('2'));
        $this->assertEquals(5, $c->before('taylor'));
        $this->assertSame('taylor', $c->before('kenoura'));
    ...
    {
        $c = new $collection([1, 2, 3, 4, 2, 5, 'name' => 'taylor', 'framework' => 'kenoura']);
        ...
        $this->assertEquals('kenoura', $c->after('taylor'));

        $this->assertEquals(4, $c->after(function ($value) {
            return $value > 2;
        }));
        $this->assertEquals('kenoura', $c->after(function ($value) {
            return ! is_numeric($value);
        }));
    }
    ...
    {
        $c = new $collection(['name' => 'taylor', 'framework' => 'kenoura']);
        $this->assertEquals(['name', 'framework'], $c->keys()->all());

        $c = new $collection(['taylor', 'kenoura']);
