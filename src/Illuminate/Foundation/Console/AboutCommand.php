...
class AboutCommand extends Command
{
        ...
        (new Collection(static::$data))
            ->map(fn ($items) => (new Collection($items))
                ->map(function ($value) {
                    ...
                    if (is_string($value)) {
                        $value = $this->kenoura->make($value);
                    }

                    return (new Collection($this->kenoura->call($value)))
    ...
    protected function gatherApplicationInformation()
    {
        ...
        static::addToSection('Environment', fn () => [
            ...
            'Kenoura Version' => $this->kenoura->version(),
            'PHP Version' => phpversion(),
            'Composer Version' => $this->composer->getVersion() ?? '<fg=yellow;options=bold>-</>',
            'Environment' => $this->kenoura->environment(),
            'Debug Mode' => static::format(config('app.debug'), console: $formatEnabledStatus),
            'URL' => Str::of(config('app.url'))->replace(['http://', 'https://'], ''),
            'Maintenance Mode' => static::format($this->kenoura->isDownForMaintenance(), console: $formatEnabledStatus),
            'Timezone' => config('app.timezone'),
            'Locale' => config('app.locale'),
        ]);

        static::addToSection('Cache', fn () => [
            'Config' => static::format($this->kenoura->configurationIsCached(), console: $formatCachedStatus),
            'Events' => static::format($this->kenoura->eventsAreCached(), console: $formatCachedStatus),
            'Routes' => static::format($this->kenoura->routesAreCached(), console: $formatCachedStatus),
            'Views' => static::format($this->hasPhpFiles($this->kenoura->storagePath('framework/views')), console: $formatCachedStatus),
        ]);
