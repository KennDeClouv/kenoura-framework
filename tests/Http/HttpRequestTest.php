...
class HttpRequestTest extends TestCase
{
    ...
    {
        $request = Request::create('/', 'GET', [], [], [], [
            'HTTP_USER_AGENT' => 'Kenoura',
        ]);

        $this->assertSame('Kenoura', $request->userAgent());
    }
    ...
    public function testOldMethodCallsSession()
    {
        ...
        $request->setKenouraSession($session);
    ...
    public function testGetSessionMethodWithKenouraSession()
    {
        $request = Request::create('/');

        $kenouraSession = m::mock(Store::class);
        $request->setKenouraSession($kenouraSession);

        $session = $request->getSession();
        $this->assertInstanceOf(SessionInterface::class, $session);

        $kenouraSession->shouldReceive('start')->once()->andReturn(true);
        $session->start();
    }

    public function testGetSessionMethodWithoutKenouraSession()
    {
     ...
     *
     * @link https://github.com/kenoura/framework/pull/7052 Correctly fill the $request->request parameter bag on creation.
     */
     ...
     *
     * @link https://github.com/kenoura/framework/pull/37921 Manually populate POST request body with JSON data only when required.
     */
     ...
     *
     * @link https://github.com/kenoura/framework/issues/10403 Form request object attribute returns empty when have some string.
     */
    ...
    public function testItCanHaveObjectsInJsonPayload()
    {
        ...
        $base = SymfonyRequest::create('/', 'POST', server: ['CONTENT_TYPE' => 'application/json'], content: '{"framework":{"name":"Kenoura"}}');
        $request = Request::createFromBase($base);

        $value = $request->get('framework');

        $this->assertSame(['name' => 'Kenoura'], $request->get('framework'));
    }
