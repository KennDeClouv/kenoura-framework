...
{
    #[WithConfig('database.connections.pgsql', ['host' => 'test-pooler.pg.kenoura.cloud', 'username' => 'test-username', 'password' => 'test-password'])]
    public function test_it_can_resolve_core_container_aliases()
    {
        Cloud::configureUnpooledPostgresConnection($this->app);

        $this->assertEquals([
            'host' => 'test.pg.kenoura.cloud',
    ...
    {
        $_SERVER['KENOURA_CLOUD_DISK_CONFIG'] = json_encode(
            [
                [
                    'disk' => 'test-disk',
        ...
        unset($_SERVER['KENOURA_CLOUD_DISK_CONFIG']);
    }
}
