...
class PendingRequest
{
    ...
    protected function sendRequest(string $method, string $url, array $options = [])
    {
        ...
        $kenouraData = $this->parseRequestData($method, $url, $options);
    ...
    protected function parseRequestData($method, $url, array $options)
    {
        ...
        $kenouraData = $options[$this->bodyFormat] ?? $options['query'] ?? [];

        $urlString = (new Stringable($url));

        if (empty($kenouraData) && $method === 'GET' && $urlString->contains('?')) {
            $kenouraData = (string) $urlString->after('?');
        }

        if (is_string($kenouraData)) {
            parse_str($kenouraData, $parsedData);

            $kenouraData = is_array($parsedData) ? $parsedData : [];
        }

        if ($kenouraData instanceof JsonSerializable) {
            $kenouraData = $kenouraData->jsonSerialize();
        }

        return is_array($kenouraData) ? $kenouraData : [];
    }
            ...
            return function ($request, $options) use ($handler) {
                ...
                return $promise->then(function ($response) use ($request, $options) {
                    $this->factory?->recordRequestResponsePair(
                        (new Request($request))->withData($options['kenoura_data']),
        ...
        return function ($handler) {
            return function ($request, $options) use ($handler) {
                $response = ($this->stubCallbacks ?? new Collection)
                    ->map
                    ->__invoke((new Request($request))->withData($options['kenoura_data']), $options)
                    ->filter()
                    ->first();
        ...
        return tap($request, function (&$request) use ($options) {
            $this->beforeSendingCallbacks->each(function ($callback) use (&$request, $options) {
                $callbackResult = call_user_func(
                    $callback, (new Request($request))->withData($options['kenoura_data']), $options, $this
                );
