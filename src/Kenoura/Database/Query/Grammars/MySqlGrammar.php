...
class MySqlGrammar extends Grammar
{
    ...
    protected function compileLegacyGroupLimit(Builder $query)
    {
        ...
        $partition = ', @kenoura_row := if(@kenoura_group = '.$column.', @kenoura_row + 1, 1) as `kenoura_row`';
        $partition .= ', @kenoura_group := '.$column;
        ...
        $from = '(select @kenoura_row := 0, @kenoura_group := 0) as `kenoura_vars`, ('.$sql.') as `kenoura_table`';

        $sql = 'select `kenoura_table`.*'.$partition.' from '.$from.' having `kenoura_row` <= '.$limit;

        if (isset($offset)) {
            $sql .= ' and `kenoura_row` > '.$offset;
        }

        return $sql.' order by `kenoura_row`';
    }
    ...
    public function compileUpsert(Builder $query, array $values, array $uniqueBy, array $update)
    {
        ...
        $columns = (new Collection($update))->map(function ($value, $key) use ($useUpsertAlias) {
            ...
            return $useUpsertAlias
                ? $this->wrap($value).' = '.$this->wrap('kenoura_upsert_alias').'.'.$this->wrap($value)
