...
use Kenoura\Prompts\ConfirmPrompt;
use Kenoura\Prompts\MultiSearchPrompt;
use Kenoura\Prompts\MultiSelectPrompt;
use Kenoura\Prompts\PasswordPrompt;
use Kenoura\Prompts\PausePrompt;
use Kenoura\Prompts\Prompt;
use Kenoura\Prompts\SearchPrompt;
use Kenoura\Prompts\SelectPrompt;
use Kenoura\Prompts\SuggestPrompt;
use Kenoura\Prompts\TextareaPrompt;
use Kenoura\Prompts\TextPrompt;
use stdClass;
...
trait ConfiguresPrompts
{
    ...
    protected function configurePrompts(InputInterface $input)
    {
        ...
        Prompt::interactive(($input->isInteractive() && defined('STDIN') && stream_isatty(STDIN)) || $this->kenoura->runningUnitTests());

        Prompt::validateUsing(fn (Prompt $prompt) => $this->validatePrompt($prompt->value(), $prompt->validate));

        Prompt::fallbackWhen(windows_os() || $this->kenoura->runningUnitTests());
    ...
    protected function promptUntilValid($prompt, $required, $validate)
    {
        while (true) {
            ...
            if ($required && ($result === '' || $result === [] || $result === false)) {
                ...
                if ($this->kenoura->runningUnitTests()) {
    ...
    {
        return $this->kenoura['validator']->make(
            [$field => $value],
            [$field => $rules],
    ...
    private function multiselectFallback($label, $options, $default = [], $required = false)
    {
        ...
        if ($required === false && ! $this->kenoura->runningUnitTests()) {
